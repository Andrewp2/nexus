...
pub fn LoginAndSignup(
    ...
    let on_submit_login =
        move |(ev, set_login_disabled): (leptos::ev::SubmitEvent, WriteSignal<bool>)| {
    ...
    let on_submit_signup =
        move |(ev, set_signup_disabled): (leptos::ev::SubmitEvent, WriteSignal<bool>)| {
            // stop the page from reloading!
            ev.prevent_default();
    ...
    view! {
        <div class="w-full flex flex-row box-border justify-evenly">
            <ActionForm action=login_action class="flex flex-col w-60" on:submit=move |e: leptos::ev::SubmitEvent| on_submit_login((e, set_login_disabled))>
            ...
            <ActionForm action=sign_up class="flex flex-col w-60" on:submit=move |e: leptos::ev::SubmitEvent| on_submit_signup((e, set_signup_disabled))>
                ...
                <div class="flex flex-col py-1">
                    <p>
                        "Password strength: "
                        {move || match zxcvbn(password().as_str(), &[]).score() {
                            zxcvbn::Score::Zero => "Very Weak",
                            zxcvbn::Score::One => "Weak",
                            zxcvbn::Score::Two => "Fair",
                            zxcvbn::Score::Three => "Good",
                            zxcvbn::Score::Four => "Strong",
                            _ => "Very Weak",
                        }}
                    </p>
                    <div class="flex justify-start items-center">
                        <div class="flex-1 rounded-lg bg-slate-400">
                            <div
                                class=move || {
                                    let password_strength: u8 = zxcvbn(password().as_str(), &[]).score().into();
                                ...
                                style=move || {
                                    let password_strength: u8 = zxcvbn(password().as_str(), &[]).score().into();
                                    let password_strength_percentage = password_strength * 25;
                                    format!("width: {password_strength_percentage}%")
                                }
                            >
                            </div>
                        </div>
